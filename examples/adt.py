...
theory NATURAL
    ...
    constant Z: Nat
    function S: Nat -> Nat
    relation subNat: Nat Nat
    relation eqNat: Nat Nat [smt("(= #1 #2)")]
end

theory BTREE extending NATURAL
    sort BTree
    constant C1: BTree
    constant C2: BTree
    function Node: BTree BTree -> BTree
    // function Leaf: Nat -> BTree
    // function Node: Nat BTree BTree -> BTree
    relation subBTree: BTree BTree
    relation eqBTree: BTree BTree [smt("(= #1 #2)")]
end
""")

nat_sort = theory_map["NATURAL"].language.get_sort("Nat")
btree_sort = theory_map["BTREE"].language.get_sort("BTree")

language = theory_map["BTREE"].language
prefix_model = build_prefix_term_model(language, 4)
...
with smt.Solver(name="z3") as solver:
    ...
    free_vars = nat_x, nat_y, nat_z, btree_tx, btree_ty, btree_tz
    ...
    precomputed_ground_terms = FOLUtils.get_ground_terms_in_language(language, 3)

    # for sort, terms in precomputed_instantiation_terms.items():
    #     print(sort)
    #     for term in terms:
    #         print("   ", term)
    # exit()
    ...
    templates = (
        # AtomicFormulaTemplate(language, (nat_x, nat_y), 0, allow_constant=True),
        # AtomicFormulaTemplate(language, (nat_x, nat_y), 1, allow_constant=True),
        AtomicFormulaTemplate(language, (btree_tx, btree_ty), 0, allow_constant=True),
        AtomicFormulaTemplate(language, (nat_x, btree_tx, btree_ty), 1, allow_constant=True),
        # Implication(
        #     AtomicFormulaTemplate(language, (nat_x, nat_y), 0, allow_constant=True),
        #     AtomicFormulaTemplate(language, (nat_x, nat_y), 0, allow_constant=True),
        # ),
        ...
        #         AtomicFormulaTemplate(language, (nat_x, nat_y, nat_z), 0, allow_constant=True),
        #         AtomicFormulaTemplate(language, (nat_x, nat_y, nat_z), 0, allow_constant=True),
        #     ),
        #     AtomicFormulaTemplate(language, (nat_x, nat_y, nat_z), 0, allow_constant=True),
        # ),
        ...
        # ),
        Implication(
            AtomicFormulaTemplate(language, (nat_x, btree_tx, btree_ty), 0, allow_constant=True),
            AtomicFormulaTemplate(language, (nat_x, btree_tx, btree_ty), 0, allow_constant=True),
        ),
        Implication(
            Conjunction.from_conjuncts(
                AtomicFormulaTemplate(language, (btree_tx, btree_ty, btree_tz), 0, allow_constant=True),
                AtomicFormulaTemplate(language, (btree_tx, btree_ty, btree_tz), 0, allow_constant=True),
            ),
            AtomicFormulaTemplate(language, (btree_tx, btree_ty, btree_tz), 0, allow_constant=True),
        ),
        ...
        # AtomicFormulaTemplate(language, (nat_x, nat_y, btree_tx, btree_ty), 2, allow_constant=True),
    )
