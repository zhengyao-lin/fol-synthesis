...
fof(axiom_13, axiom, ! [X, B, A] : (union(union(B, concat(A, X)), X) = X => union(concat(closure(A), B), X) = X)).
fof(axiom_14, axiom, ! [X, B, A] : (union(union(B, concat(X, A)), X) = X => union(concat(B, closure(A)), X) = X)).

fof(goal, conjecture, ! [B] : union(closure(zero), closure(B)) = closure(union(one, B))).
